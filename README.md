<p align="center">
  <img src="githubBanner.png" alt="Eloy Fernández Banner" width="100%" height="70%">
</p>

# Hi! I´m Eloy Fernández Torres 

I am a responsible and punctual individual with a strong desire to learn and grow. I am currently seeking for a new professional experience in Web Application Development and the tech industry, where I can contribute my positive attitude, energy, and commitment. I adapt quickly to new environments, enjoy teamwork, and maintain a mindset of continuous improvement to grow in every project


### 💻 Technical Skills
* **Front-End:** JavaScript, HTML, CSS, Bootstrap
* **Back-End:** Spring Boot with Java.
* **Other Languages:** Python.
* **Databases:** MySQL, ObjectDB.
* **Core Technologies:** REST APIs.
* **Digital Competencies:** Fluent use of web tools and social media, with a high capacity to learn new technologies rapidly
### 🛠️ Tools & DevOps
* **Version Control:** Git & GitHub.
* **IDE/Editors:** VS Code, IntelliJ IDEA.
* **Environment:** Windows.
* **Design & Productivity:** Web tools and social media management.
### 📚 Education & Certifications
* **Higher Technical Certificate (GS) in Web Application Development (DAW)** 
    * *Colegio Bilingüe Valle del Miro, Valdemoro* (09/2024 – 03/2025).
* **Baccalaureate in Social Sciences** 
    * *IES Avalon, Valdemoro* (09/2022 – 06/2024).
* **Languages:**
    * **Spanish:** Native.
    * **English:** B2-C1 Level.
### 🧠 Soft Skills & Aptitudes
* Excellent communication and interpersonal skills.
* Teamwork, organization, and responsibility.
* Problem-solving and proactive attitude.
* Punctuality, commitment, and schedule flexibility.
### 🚀 Professional Experience
**SAN JORGE TECNOLOGÍAS | Software & Systems Developer (Internship)** *Valdemoro, Spain | March-2026 - July-2026. *Strategic tech consultancy collaborating with the European Union (EU) and Law Enforcement Agencies (Civil Guard) on critical infrastructure, defense, and security projects.*

* **Robotics & Sensor Integration:** Developed and deployed robotic controllers and sensor monitoring systems under high-availability protocols.
* **Secure Communications:** Implemented Tunneling technologies for network data management, ensuring integrity and security in remote monitoring.
* **Full-Stack Development & APIs:** Designed Frontend and Backend applications with advanced integration of external APIs to optimize data flows.
* **Internal Software:** Created local network tools aimed at improving productivity and digitizing company operational processes.
* **Gamification & Web Presence:** Developed the corporate web platform, integrating educational video game modules for technical marketing and training purposes.
<!--### 📂 Github
[![GitHub Streak](https://github-readme-streak-stats.herokuapp.com?user=Eloy-Fernandez-Torres&theme=merko&border_radius=6.3)](https://git.io/streak-stats) -->
### 📫 Contact Me
* **Location:** Valdemoro, Madrid, Spain.
* **Email:** [eloy.fernandez.torres1@gmail.com](mailto:eloy.fernandez.torres1@gmail.com).
* **Phone:** 601 20 86 05.
